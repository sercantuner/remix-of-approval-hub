...
serve(async (req) => {
  ...
  try {
    ...
    // Port 25 typically no TLS
    const useTls = smtp_port === 465 ? true : false;
    
    console.log(`[test-smtp] Connecting to ${smtp_host}:${smtp_port} with TLS=${useTls}, secure_flag=${smtp_secure}`);
